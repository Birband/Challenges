...
def calculate_pheromones_and_best_path(ants, pheromones, evaporation):
    updated_pheromones = {
        edge: pheromone * evaporation for edge, pheromone in pheromones.items()
    }
    best_ant = None
    ...
    if best_ant:
        for i in range(len(best_ant.visited_nodes) - 1):
            ...
            updated_pheromones[edge] = (
                updated_pheromones.get(edge, 0) + 1 / best_path_length
            )

    return (
        updated_pheromones,
        best_path_length,
        best_ant.visited_nodes if best_ant else [],
    )
...
def main():
    ...
    while running:
        ...
        if pygame.time.get_ticks() % TRAIL_PERIOD == 0:
            for i, trail_segment in enumerate(trail):
                for j, point in enumerate(trail_segment):
                    ...
                    pygame.draw.circle(
                        trail_surface, (255, 255, 255, opacity), (2, 2), 2
                    )
        ...
        if completed_ants == len(ants):
            pheromones, path, path_nodes = calculate_pheromones_and_best_path(
                ants, pheromones, EVAPORATION
            )
            if path < best_path:
                best_path = path
        ...
        if best_path_nodes:
            ...
            for i in range(len(best_path_nodes) - 1):
                pygame.draw.line(
                    screen,
                    (0, 255, 0),
                    (best_path_nodes[i].x, best_path_nodes[i].y),
                    (best_path_nodes[i + 1].x, best_path_nodes[i + 1].y),
                    2,
                )
